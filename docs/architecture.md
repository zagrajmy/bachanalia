...
[`IN_PROGRESS.md`](../IN_PROGRESS.md) for why, and what is left to wire.
...
- **Do not hand-edit WooCommerce gateway positions.**
  `WC_Payment_Gateways::init()` indexes gateways _by_ position, so a repeated
  number deletes one from the store outright.
  `wordpress/bachanalia-paynow-first/` filters `woocommerce_gateway_order`
  instead — the one input both the classic and the Blocks checkout read.
- **WooGraphQL's session settings must be saved twice.** The four
  `*_nonce_param` inputs render disabled until a URL field is enabled, and a
  disabled input is not submitted, so the save that ticks the checkboxes
  cannot write them. Until those options exist
  `Protected_Router::get_nonce_names()` returns only `download_url`, the
  plugin never looks for the `_wc_checkout` our URL carries, and it gives up
  before checking the nonce. Every failure in that handler exits through the
  same `status_header( 404 )` and redirect home, so a broken handoff and a
  missing endpoint are indistinguishable from outside. Ours looked like a
  missing endpoint for a day.
- **Cloudflare fronts the site** and challenges document requests from
  automated clients, returning 403 on HTML while `/graphql` and `/wp-content`
  keep answering. It expires on its own, and it is Cloudflare rather than
  Wordfence — the plugin's own thresholds were checked and ruled out.

## Security

The incident history, the credential state, the hosting account and what is
still owed on all three live in the runbook, which is private. This repo is
not.

One rule belongs with the code: **WooGraphQL installs from a GitHub zip**, off
the wordpress.org registry, so it gets no auto-updates. Several other plugins
here are the same. An unmaintained plugin has already cost this site once, so
updating them is a habit somebody keeps, not a background process.

## Tooling

- `playwriter` drives wp-admin. Its `page.click()` on WordPress settings forms
  crashes the relay mid-navigation; use `form.requestSubmit()` inside
  `page.evaluate()` and verify out-of-band.
